...
formula += 'Из каждого ящика вытаскивают случайным образом по два мяча. Какова вероятность того, что: a) все мячи белые? \n'
...
formula += '$\\ {m_1} $'
formula += ' количество благоприятных исходов, извлечений из первого ящика 2-х белых мячей из 7 \n'
formula += '$\\ {m_1} : \\ C^k_n = С^{2}_{7} $ \n'
formula += '$\\ {n_1} $'
formula += ' количество извлечения из первого ящика 2-х мячей из 10. \n'
formula += '$\\ {n_1} : \\ C^k_n = С^{2}_{10} $\n'
formula += '$\\ {m_2} $'
formula += ' количество благоприятных исходов, извлечений из второго ящика 2-х белых мячей из 9 \n'
formula += '$\\ {m_2} : \\ C^k_n = С^{2}_{9} $ \n'
formula += '$\\ {n_2} $'
...
P2 = m2 / n2
P = P1*P2
formula += 'Вычислим  $\\ {P(A)} \\ {и} \\ {P(B)} \\ {и} \\ {P} $ \n'
formula += '$\\ {P(A)}=' + str(round(P1,4)) + ' \\ {и} \\ {P(B)}=' + str(round(P2,4)) + ' \\ {и} \\ {P}=' + str(round(P,4)) + ' $ \n'
